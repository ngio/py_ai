""" 유튜브 자막 가져오기, 주소 입력받기, 자동 생성 자막 세그먼트도 같이 출력, TXT파일로 저장
...
import os # 파일 경로 및 존재 여부 확인을 위한 모듈
...
if __name__ == "__main__":
    ...
    if target_video_id:
        ...
        if transcript_data:
            ...
            # 전체 자막 텍스트 생성 (줄 바꿈 적용)
            # 각 세그먼트의 텍스트를 줄 바꿈 문자('\n')로 연결하여 전체 텍스트 생성
            full_text_with_newlines = "\n".join([segment.text for segment in transcript_data])
            print(f"\n--- 전체 {transcript_type} 자막 텍스트 (앞부분만, 줄 바꿈 적용) ---")
            print(full_text_with_newlines[:1000] + "...") # 앞부분 1000자만 출력

            # --- TXT 파일로 저장 기능 추가 ---
            # 파일명 설정 (예: [동영상ID]_자막.txt)
            file_name = f"{target_video_id}_caption.txt"
            # 파일이 저장될 경로 설정 (현재 스크립트가 실행되는 디렉토리)
            ...
            try:
                ...
                print(f"\n성공적으로 자막을 '{file_path}' 에 저장했습니다.")
            except Exception as e:
                print(f"\n자막 파일 저장 중 오류가 발생했습니다: {e}")
            # --- TXT 파일 저장 기능 끝 ---
